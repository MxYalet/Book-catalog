...
public class BookCatalog {
	...
	public void addBook(Book book) {
		...
		// Check if the book and its title are not null
		if (book != null && book.getTitle() != null) {
			// Add the book to the map using its title as the key
	...
	public Book getBookByTitle(String title) {
		...
		// Retrieve the book using the title as the key from the map
	...
	public Book getBookByAuthor(String author) {
		...
		// Use Java streams to iterate through the values of the bookMap
		return bookMap.values().stream()
				// Filter the books based on author (ignoring case)
				.filter(book -> book.getAuthor().equalsIgnoreCase(author))
				// Return the first matching book, or null if no match is found
				.findFirst()
				.orElse(null);
