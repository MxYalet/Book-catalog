...
public class Book {
	...
	private void setTitleAndAuthor() {
		...
		// Define patterns for matching title and author lines
		...
		for (String line : lines) {
			// Match the line against the title pattern
			Matcher titleMatcher = titlePattern.matcher(line);
			if (titleMatcher.find()) {
				// Extract the title from the matched group and trim leading/trailing whitespace
				this.title = titleMatcher.group(1).trim();
			} else {
				// Match the line against the author pattern
				Matcher authorMatcher = authorPattern.matcher(line);
				if (authorMatcher.find()) {
					// Extract the author from the matched group and trim leading/trailing whitespace
	...
	private void countWords() {
		...
		// Initialize the map to store word counts
	// Create a TreeMap to store word counts, ignoring case
		this.wordCounts = new TreeMap<>(String.CASE_INSENSITIVE_ORDER);

	// Iterate through each line in the book
		lines.forEach(line -> {
			// Split the line into words using non-word characters as delimiters
			String[] words = line.split("[^a-zA-Z0-9_%+-]+");
			// Count occurrences of each word and update the wordCounts map
	...
	// Calculate the total word count using Java streams
		this.wordCount = wordCounts.values().stream().mapToInt(Integer::intValue).sum();
	}
	...
	public int getSpecificWordCount(String word) {
		...
		return this.wordCounts.getOrDefault(word.toLowerCase(), 0);
	}
